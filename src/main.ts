...
import CopyVerseModal, {
	LinkType as CopyLinkType,
} from "src/modals/copy-verse-modal";
...
export default class BibleLinkerPlugin extends Plugin {
	...
	openCopyModal = () => {
		...
		if (view) {
			new CopyVerseModal(this.app, this.settings, (str) =>
	...
	openObsidianLinkModal = () => {
		const view = this.app.workspace.getActiveViewOfType(MarkdownView);
		if (view) {
			new LinkVerseModal(this.app, this.settings, (str) =>
				view.editor.replaceRange(str, view.editor.getCursor())
			).open();
		}
	};
