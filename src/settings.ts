...
export class SettingsTab extends PluginSettingTab {
	...
	display() {
		...
		containerEl.createEl("h2", {
			text: "Copy and Link Bible verses command",
		});
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Change this if wrong verses are being linked, e.g. you want "Gen 1,1-3" but output is text from verses 2-4 → set this to -1'
			)
				...
				inputBox
					...
					.onChange(async (value) => {
						...
						if (Number.isNaN(number)) {
							new Notice(
								"Invalid input, please insert valid integer"
							);
							inputBox.setValue("");
							return;
						}
		...
		new Setting(containerEl)
			...
			.setDesc(
				'If set, only headings of specified level are considered verses (if first heading of this level is always a verse, also set "Verse offset" to -1)'
			)
			.addDropdown((dropdown) => {
				...
				dropdown.setValue(
					this.plugin.settings.verseHeadingLevel?.toString() ?? "any"
				);
				dropdown.onChange(async (value) => {
					this.plugin.settings.verseHeadingLevel =
		...
		new Setting(containerEl)
			.setName("Line prefix")
			.setDesc(
				"String inserted in front of every line, for example '>' for quote. Note: If you set 'Put each verse on a new line?' to true, the prefix will be inserted in front of every line."
			)
			.setClass("important-setting")
			.addText((inputBox) =>
				inputBox
		...
		new Setting(containerEl)
			...
			.setDesc(
				"String inserted after biblical link, you can use \\n to insert newline."
			)
			.addText((inputBox) =>
				inputBox
		...
		new Setting(containerEl)
			...
			.setDesc(
				'String inserted in front of every copied verse. You can use "{n}" where you want number of given verse inserted, for example "**{n}** " will make each verse start with bold verse number. You can also use "{f}" to insert name of the corresponding file (for example to create obsidian links). Leave empty for no prefix.'
			)
			.addText((inputBox) =>
				inputBox
					.setPlaceholder("Insert prefix here")
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Should last verse be linked in the visible link before text of verses?"
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Invisible links are added to each verse used (so you can find the connections later), they are only visible in source mode."
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Should space be inserted between verses? (Only applied when Put each verse on a new line? is se to false. Useful for languages such as Chinese.)"
			)
		...
		new Setting(containerEl)
			...
			.setDesc(
				'This is the symbol that will be used between chapter number and verse number when copying one verse. For example "." → Gen 1.1.'
			)
			.addText((inputBox) =>
				inputBox
		...
		new Setting(containerEl)
			...
			.setDesc(
				'This is the symbol that will be used between chapter number and verse number when copying multiple verses. For example "," → Gen 1,1-3.'
			)
			.addText((inputBox) =>
				inputBox
					.setPlaceholder("Insert notation symbol here")
		...
		if (this.plugin.settings.enableMultipleTranslations) {
					...
					inputBox
						...
						.onChange(async (value) => {
							...
							inputPaths.forEach((path) => {
								// parse user input for later use
								if (path.at(-1) !== "/") {
									// Add potentionally missing '/' to path
									paths.push(path + "/");
								} else {
									paths.push(path);
								}
			...
			new Setting(containerEl)
				...
				.setDesc(
					"Choose what translations should be linked when copying a verse."
				)
				.addDropdown((dropdown) => {
					...
					dropdown.addOption(
						"usedAndMain",
						"Link to used and main translation"
					);
					dropdown.addOption("main", "Link only to main translation");
					dropdown.setValue(
						this.plugin.settings.translationLinkingType
					);
					dropdown.onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"This is the separator that will be used when linking, e.g. if you enter '#' here, output will be [[Gen 1#1]]. If you are using headings to mark verses, use '#'. If you are using block references, use '^'."
			)
			.setClass("important-setting")
			.addText((inputBox) =>
				inputBox
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Fill this if you are using verse prefixes in your bible files, e.g. you have "v1" in your file → set to "v".'
			)
			.setClass("important-setting")
			.addText((inputBox) =>
				inputBox
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Verify existence of files you are trying to link, so that you are not inserting wrong references by mistake."
			)
			.addToggle((toggle) =>
				toggle
