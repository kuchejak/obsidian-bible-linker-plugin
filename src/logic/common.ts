 ...
 */
export function getFileByFilename(app: App, filename: string) {
	let filenameCopy = filename;
	let tFile = app.metadataCache.getFirstLinkpathDest(filenameCopy, "/")
	if (!tFile) { // handle "Bible study kit" file naming, eg. Gen-01 instead of Gen 1
		filenameCopy = tryConvertToOBSKFileName(filenameCopy);
		tFile = app.metadataCache.getFirstLinkpathDest(filenameCopy, "/");
	}
